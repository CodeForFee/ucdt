...
- **Heat reshape (fix from T-007 review):** legacy `/api/heat` returned `heatController`'s reshape of `getHeatData` (controllers/heat.controller.ts:18-40), not the service output. `snapshots.heat_payload` applies it (`city` = `settings.city_id`). Checked every other `controllers/*.controller.ts`: weather/aqi/flood/recommend pass the service result through unchanged, so those snapshots were already right.
...
- Heat `result` is the controller shape: the worker test expects heatController's reshape of the fixture's getHeatData output. Latest dev-DB snapshots of all five hazards validate against T-007's `WeatherData/AQIData/FloodData/HeatData/RecommendData` (models.py from origin/feat/T-007-climate-api-contracts).
...
```
After the heat reshape: `tests/ingest tests/worker` 21 passed in 7.38s; ruff clean; `uv run pytest -q` 144 passed in 43.70s.
...
(third run, after the heat fix, 18:25:24Z: each hazard count 3; heat id 14 keys = city, timestamp, avgTemperature, maxTemperature, heatIslandIntensity, hotspots, geojson; all five latest rows validate against T-007's models)
...
- **Dev DB:** the two heat snapshots written before the fix (runs at 18:20Z and 18:21Z) still have the raw getHeatData shape. `latest` is fine; `/v1/history/heat` over that window would hit them. I left them in place: delete them (`delete from risk_snapshots where hazard='heat' and computed_at < '2026-09-23 18:25Z'`) or ignore.
