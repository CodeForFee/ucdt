...
## Bugs
- [ ] B-020 Heat simulation preview card compares two different quantities: its circles show `avgTemperature` (measured AIR temperature, e.g. 27.7°) and `avgTemperature + tempDelta`, while the zone list under it shows each cell's EFFECTIVE temperature (heat index + UHI, ~35°) and `tempDelta` itself is an effective-temperature delta. Found 2026-09-24 claude-opus-5 while fixing B-019 (legacy behaviour, same in Hackathon-FE). Fix needs a choice of which quantity the card headlines; not scheduled.
